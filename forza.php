<!DOCTYPE html>
<html lang="pt-br">

<head>
    <script type="text/javascript" src="jquery.min.js"></script>
    <script type="text/javascript" src="jquery.cycle.all.js"></script>
    <script type="text/javascript">
     	$(document).ready(function() {
			    $('#divbn').cycle({
					fx: 'scrollLeft',
					timeout:500    
                });
            });
                
    </script>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="stylejgs.css">
    <title>Document</title>
</head>

<body bgcolor="lightgrey">

    <header class="body_full">
        <div id="div1">
            <img src="./img/logo.png" alt="" style="max-width: 200px;" height="133px">
            <!-- <h1>
                VueJs
            </h1> -->
        </div>
        <!-- <div id="divBusca">
            <img src="./img/lupa.png" alt="Buscar..." height="30px">
            <form action="www.google.com" method="post">
                <input type="text" id="txtBusca" placeholder="Buscar..." />

                <input type="submit" id="btnBusca" value="Buscar">
            </form> -->
        </div>
        <nav>
            <ul class="link_link">
                <li>
                    <div style="text-align: center;">
                        <img src="./img/user.png" alt="" style="max-width: 30px;">
                    </div>
                    <a style="color:rgb(223, 223, 223) ;" href="index.php">Início
                    
                </li>
                <li>
                    <div style="text-align: center;">
                        <img src="./img/user.png" alt="" style="max-width: 30px;">
                    </div>
                    <a style="color:rgb(238, 238, 238) ;" href="cadastro.php">Cadastro
                    
                </li>
                <li>
                    <div style="text-align: center;">
                        <img src="./img/user.png" alt="" style="max-width: 30px;">
                    </div>
                    <a style="color:rgb(252, 246, 246) ;" href="login.php">
                    Login</a>
                </li>
                <li>
                    <div style="text-align: center;">
                        <img src="./img/iconint.png" alt="" style="max-width: 28px;">
                    </div>
                    <div style=" text-align: center;">
                        <a style="color:rgb(250, 246, 246) ;" href="sobre.php">About</a>

                    </div>
                </li>
                <li>
                    <div style="text-align: center;">
                        <img src="./img/tel.png" alt="" style="max-width: 26px;">
                    </div>
                    <a style="color:rgb(233, 223, 223) ;" href="#">Contact</a>
                </li>
            </ul>

        </nav>
    </header>
    

    
    
    <!-- <div style="background-color: black;" id="divl">
       
        <a href="www.google.com">
        <img class="imganun" src="./img/cps.png">
    </a>

    <a href="www.google.com">
    
        <img class="imganun" src="./img/cxb.png" alt="">
    </a>
    <a href="www.google.com">   
    <img id="imgdivl" src="./img/cnin.png" alt="">
    </a>
    </div> -->
    
    
    <div style="display: flex;">
        <div class="styleBox" style="border: 1px solid #000;">
            <div>
                <img style="align-self: flex-start " src="./img/forza.jpg" alt="" height="100%" width="100%">
            </div>
            <!-- <div class="box_game" style="display: block;">
                <div class="title_game">The Last Of Us 2 - PS4</div>
                <div class="valor"> R$ 200,00</div>
                <div><span class="parcela"> até 4x de R$400 sem juros</span></div>
                <div><span class="fpag"> ou <span class="valor_fpag"> R$ 284,90 </span> via pix</span></div>
                <button class="btn">Comprar</button>
            </div> -->
            
            <a class="btntop" href="#">
       
                <img style="width: 100px; background-color: red;" src="./img/icon.png" alt="">        
            </a>
        
        </div> 
        
        <div class="styleBoxt" style="border: 1px solid #000;">
            
            <div class="box_game" style="display: block;">
                <div class="title_game">Forza Horizon 5 - Xbox</div>
                <div><span class="valor_fpag"> Marca: Xbox Game Studios </span> <span class="fpag"> Cód: RTDSFVEWF </span> </div>
                <div class="valor"> R$ 250,00</div>
                <div><span class="parcela"> até 4x de R$62,50 sem juros</span></div>
                <div><span class="fpag"> ou <span class="valor_fpag"> R$ 200,00 </span> via pix</span></div><br>
                <a style="color: red;" href="omowdf">Assistir ao trailer</a><br>
                <button  class="btn">Comprar</button> 
                
            </div> 
            <a class="btntop" href="#">
       
                <img style="width: 100px; background-color: red;" src="./img/icon.png" alt="">        
            </a>
        
        </div>
        
    </div>
        
         
       
     
        
        
    </div>
    <div class="styleBoxtt" style="border: 1px solid #000;">
            
        <div class="box_game" style="display: block;">
            
            <div class="title_game">Descrição</div>
            <hr> 
            <br>
            
            <div><span  class="valor_fpag1"> Especificações </span> </div>
             <div><span class="fpag2">Plataforma: Xbox<br>

                Formato: Mídia Digital Envio Gratuito Por E-mail
                <br>
                Gênero: Corrida
                <br>
                Produtora: Playground Games Turn 10 Studios
                <br>
                Idioma: Português
                <br>
                Legenda: Português
                <br>
                
                
                Data de Lançamento: 1 Novembro 2021 </span> </div> <br>
                <div><span  class="valor_fpag1"> Descrição do jogo </span> </div>
                <div><span class="fpag2">Forza Horizon 5 é um jogo eletrônico de corrida ambientado em um ambiente de mundo aberto sediado no México. O jogo tem o maior mapa de toda a série Forza Horizon, sendo 50% maior do que o de Forza Horizon 4, além de também ter o ponto mais alto da série Horizon.<br>

                     </span> </div> <br>
            
            
            
            
            
            
        </div> 
        <a class="btntop" href="#">
   
            <img style="width: 100px; background-color: red;" src="./img/icon.png" alt="">        
        </a>
    
    </div>
    
     <footer class="div_full">
        <div class="div_rdp">
           <img src="./img/logo.png" alt="" style="max-width: 150px;" height="133px">
           Blaze Store
        </div>
        <div class="div_rdp1">
            Atendimento: (21) 968809708
            blazestoresupport@gmail.com
<br> <br>
            Horário de atendimento <br>
Seg a Sex de 08h às 00hs <br>
Sáb a Dom de 10h às 23hs
        </div>
        <div class="div_rdp2">
            GUILHERME DE OLIVEIRA 43408816867 | <br> CNPJ: 29.135.352/0001-36 |  <br> RUA MARCOS CALHEIRO, 543 AP- CIDADE BANGU - RJ CEP 08583-360
        </div>
        <div class="div_rdp3">
            <!-- <div style="background-color: red; align-self: center;">
            <a href="www.google.com">
            <img src="./img/logo.png" alt="" style="max-width: 150px;" height="133px">
            </a> -->
        
    </div>
   
</div>

</footer>
    

    <!-- <div id="divlat">
        <fieldset>
        <h2 style="font-family: Verdana, Geneva, Tahoma, sans-serif; padding-left: 10px; color: rgb(0, 0, 0) ; padding-bottom: 10px;">Plataformas</h2>
        
            <a class="linkp" href="Google.com">Ps4</a>
            <br>
            
            <a class="linkp" href="Gsde.com">Xbox One</a>
            <br>
            
            <a class="linkp" href="you.com">Nintendo Wii</a> 
            <br>
        </fieldset>
            <h2 style="font-family: Verdana, Geneva, Tahoma, sans-serif; padding-left: 10px; color: rgb(19, 18, 18) ; padding-bottom: 10px;">Categoria</h2>
        
        </form>
    </div> -->

</body>

</html>